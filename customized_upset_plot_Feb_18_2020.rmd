...
data1_full <- read_csv("data1.csv") #>2RPM loci in ovary 
data2_full <- read_csv("data2.csv") #egg > 2RPM
data3_full <- read_csv("data3.csv") #seedling > 2RPM
data4_full <- read_csv("data4.csv") #sperm > 2RPM
...
upperright <- get_legend(
  ...
  geom_text(aes(label = as.character(sizes)), size = 5, angle = 90, hjust = 0, y = 1, fontface = "bold") +
  scale_fill_manual(values = c("orangered3", "tomato1", "seagreen", "dodgerblue2"), #this is my own custom palette  
                     limits = c("ovary", "egg", "seedling", "sperm")) +   #change legend text here!     
  scale_x_discrete(labels = NULL) +
  scale_y_continuous(labels = NULL) +
  labs(x = NULL,
       y = "set size",
